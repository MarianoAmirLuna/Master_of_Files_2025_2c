...
void loguear_tabla_de_paginas(t_list* tabla_paginas)
{
    for (int i = 0; i < list_size(tabla_paginas); i++)
    {
        entrada_tabla_pags *entrada = list_get(tabla_paginas, i);
...
void loguear_tabla_paginas_global()
{
    ...
    loguear_tabla_de_paginas(tabla_pags_global->elements);
